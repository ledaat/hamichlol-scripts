...
def query(request):
    ...
    while True:
        ...
        # Call API
        time.sleep(1)
        result = requests.get("https://jewiki.org.il/w/api.php", params=req).json()
        if "error" in result:
...
for result in query(
    {
        ...
        "gcmtitle": "קטגוריה:ערכים שנוצרו באנציקלופדיה היהודית",
        "prop": "info",
    }
):
    for page in result["pages"]:
        ...
        for p in revision["query"]["pages"]:
            ...
            print(title + "|" + str(size) + "|" + timestamp)
